...
class TransactionService():
    ...
    def get_transactions_by_account(self, account: str):
        '''
        print 
        '''
        ...
        while(node != None):
            if node.value.from_account == account or node.value.to_account == account:
                check = True
                node.value.display_info()
                print('\n')
    ...
    def get_transactions_by_date(self, date: str):
        ...
        while(node != None):
            data = node.value.timestamp.strip().split(' ')
            if data[0] == date:
                check = True
                node.value.display_info()
                print('\n')
...
if __name__ == "__main__":
    ...
    sv.get_transactions_by_date("29/05/2026")
